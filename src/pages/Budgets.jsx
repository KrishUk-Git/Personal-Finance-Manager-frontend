...
const Budgets = () => {
    ...
    useEffect(() => {
        if (user?.notificationsEnabled && budgets.length > 0 && transactions.length > 0) {
            budgets.forEach(budget => {
                const spending = getCategorySpending(budget.category);
                if (spending > budget.amount) {
                    showNotification(`You've exceeded your budget for ${budget.category}!`);
                }
            });
        }
    ...
    const showNotification = (message) => {
        ...
        if (Notification.permission === 'granted') {
            ...
            Notification.requestPermission().then(permission => {
                if (permission === 'granted') new Notification('Budget Alert', { body: message });
            });
        }
    };
